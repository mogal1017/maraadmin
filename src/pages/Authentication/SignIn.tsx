...
import samara from '../../images/logo/samara.svg';
 ...
 const dispatch = useDispatch();
  ...
  const handleSignin = async (event: { preventDefault: () => void; }) => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
      setEmailError('Please Eter A Valid Email Address');
      return;
    }
    ...
    try {
      ...
      const response = await postData('auth/logincmsUser', payload);
  ...
  const checkEmailExists = async (email) => {
    try {
        ...
        if (apiResponse.code === 1) {
            // Email exists in the master list
            return true;
        } else {
            // Email does not exist in the master list
            return false;
        }
    } catch (error) {
...
//     console.log("Detilss",decodedToken);
...
const responseMessage = async (response) => {
  try {
      console.log(response);
      const decodedToken = jwtDecode(response.credential);
      console.log("Details", decodedToken);

      // Check if the email exists in your master list
      const emailExists = await checkEmailExists(decodedToken.email);
      ...
      if (emailExists) {
          ...
          // Email doesn't exist, show alert and navigate to sign-in page
          alert("User is not authenticated to access this site. Please sign up.");
          navigate('/auth/signin'); // Adjust this according to your routes
      }
  } catch (error) {
      console.error("Error in responseMessage:", error);
      // Handle error appropriately, e.g., show alert or redirect to an error page
  }
};
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark ">
        <div className="flex flex-wrap items-center ">
          <div className="hidden w-full xl:block xl:w-1/2 ">
            <div className="py-17.5 px-25 text-center">
              <Link  to="/">
                {/* <img className="hidden dark:block" src={Logo} alt="Logo" />
                <img className="dark:hidden" src={LogoDark} alt="Logo" /> */}
                <img className="dark:hidden w-80 h-40 ml-auto mr-30 mb-0"  src={samara} alt="Logo" />
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              ...
              <form >
                ...
                <div className="mb-6">
                  ...
                  <div className="relative">
                    ...
                    {credentialError && <div className="text-red-500">{credentialError}</div>}
              ...
              <GoogleLogin onSuccess={responseMessage} size='large' text="signin_with" shape='pill'></GoogleLogin>
            </div>
