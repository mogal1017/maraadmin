...
export const Clinic = () => {
  ...
  const fetchData = async (pageNumber: number) => {
    try {
      ...
      const search = {
        field_name: ['name', 'email', 'address_ype', 'zipcode'],
  ...
  const debouncedGetListing = useCallback(debounce(fetchData, 500), [
    searchKeyword,
  ]);
  ...
  const columns = [
    ...
    {
      name: 'Address Type',
      selector: (row: { address_ype: any }) => row.address_ype,
      sortable: true,
    },
    {
      name: 'Zip Code',
    ...
    },
    {
      ...
      cell: (row: { id: any }) => (
        <div className="flex items-center space-x-3.5">
          ...
          <button
            className="hover:text-primary"
            onClick={() => handleNavigateUpdate(row.id)}
          >
            <svg
  ...
  const handleViewClick = (clinicId: number) => {
    const selectedClinic = clinicData.find((clinic) => clinic.id === clinicId);

    navigate(`/Clinic/ClinicView/${clinicId}`);
  };

  const handleNavigateUpdate = (clinicId: number) => {
    const selectedClinic = clinicData.find((clinic) => clinic.id === clinicId);

    navigate(`/Clinic/ClinicEdit/${clinicId}`);
  };
  ...
  return (
    <DefaultLayout>
      {loading ? (
        ...
        <>
          <Breadcrumb pageName="Clinic List" />
