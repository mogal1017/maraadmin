...
const Practitioners = () => {
  ...
  const currentRecords = practitionerData.slice(
    indexOfFirstRecord,
    indexOfLastRecord,
  );
  ...
  }, [currentPage, recordsPerPage, practitionerData.length]);
  ...
  useEffect(() => {
    setPractitionerData((prevPractionerMaster) =>
      prevPractionerMaster.map((user) => ({
        ...user,
      ...
      cell: (row) => (
        <div className="flex items-center space-x-3.5">
          <button
            ...
            onClick={() =>
              navigate(`/practitioners/viewpractitioners/${row.id}`)
            }
          >
            <svg
  ...
  const getPractitionerListing = async (pageNumber: number) => {
    try {
      ...
      const search = {
        field_name: ['f_name', 'email', 'l_name', 'type', 'npi_number'],
      ...
      if (pageNumber === 1) {
        ...
        setPractitionerData((prevPractionerMaster) => {
          const newData = response.data.filter(
            (newItem: { id: any }) =>
              !prevPractionerMaster.some(
                (existingItem) => existingItem.id === newItem.id,
              ),
          );

          return [...prevPractionerMaster, ...newData];
        });
      }
  ...
  const handleIsActiveChange = async (id: any, checked: boolean) => {
    try {
      ...
      setPractitionerData((prevData) =>
        prevData.map((practitioner) => {
          if (practitioner.id === id) {
            return { ...practitioner, is_active: response.data.is_active };
          }
          return practitioner;
        }),
      );
  ...
  return (
    <DefaultLayout>
      {loading ? (
        ...
        <>
          <Breadcrumb pageName="Practitioners List">
            <a onClick={() => navigate(`/practitioners`)}></a>
          </Breadcrumb>
